...
- Gateway production transports、agent-facing wrapper/spool/sidecar、Web UI、replay、team-server、enterprise control plane 仍是未实现面；当前 gateway crate 不执行真实 MCP/HTTP/CLI wrapper/SDK/webhook/live GitHub/secret-broker tool action，只提供 typed mediation、配置化工具策略/审批证据、known-tool registry 合同、redacted secret reference 合同、deterministic local fixture adapter、MCP/HTTP request normalization stub 和显式 unsupported protocol 合同。
