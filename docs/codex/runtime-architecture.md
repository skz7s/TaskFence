...
- 任务文件中的 `permissions.tools.allow`、`permissions.tools.approval_required` 和 `permissions.tools.deny` 已解析进内建 policy；当前 gateway crate 可以对规范化的 `mcp` tool action 做配置化 policy decision，并将 `PolicyDecision` 写入结构化 audit/report evidence；本地 executable gateway 路径会从 `gateway.tools` 构造 known-tool registry，未注册 tool action 会在 policy evaluation 前 fail closed 并写入 audit error；approval-required fixture tool call 会写入 `ApprovalRequested` / `ApprovalResolved`，审批拒绝或超时会在 adapter 执行前 fail closed；gateway secret reference 会在 policy、registry、approval 之后作为 redacted parameter 附加，不读取或使用真实凭证。
- 当前 CLI 已实现 `taskfence gateway call <task-file> <tool> <operation>` 本地 fixture 执行入口，可执行 task file 中配置的 deterministic GitHub-shaped `github.read_issue` 与 `github.create_pr` fixture：前者读取本地 JSON issue，后者在显式 approval 后写入 PR proposal artifact。该入口写入 `.taskfence/tasks/<task-id>/` 结构化 evidence 和 Markdown report，不启动 Docker，不访问网络，不调用真实 GitHub，不读取 raw token，也不是 production MCP/HTTP server。
...
- Gateway production transports、sidecar/listener、Web UI、replay、team-server、enterprise control plane 仍是未实现面；当前 gateway crate 不执行真实 MCP/HTTP/SDK/webhook/live GitHub/raw secret-broker tool action，只提供 typed mediation、配置化工具策略/审批证据、known-tool registry 合同、redacted secret reference 合同、deterministic local fixture adapter、agent-facing spool prototype、MCP/HTTP request normalization stub 和显式 unsupported protocol 合同。
